...
Проект использует **двухуровневую стратегию кэширования**. Для примера кеширование сущности `Post`:

1. **Spring Cache (аннотации)**  
   - `@Cacheable`, `@CacheEvict` в `SimplePostService`
   - Автоматическое кэширование для стандартных запросов:
     - `postList` — список всех постов  
     - `postItem` — отдельный пост  
     - `postImage` — изображения постов  
     - `userCreatedPosts`, `userFavouritePosts` — пользовательские подборки  

2. **AOP-обновление кэша**  
   - `PostCachingAspect` перехватывает `update()` и `save()`  
   - Обновляет кэшированные посты точечно, без полного сброса  
   - Очищает только кэши пользователей, связанных с изменённым постом  
