...
public class PostController {
	...
	public static final String RESPONSE_DESC_IF_POST_PAGE_RECEIVED = """
			* `totalElement` - Общее кол-во элементов.
			* `totalPages` - Общее кол-во страниц.
			* `size` - Максимальное число элементов,
			которые могут содержаться на одной странице.
			* `number` - Номер страницы.
			* `numberOfElements` - Кол-во элементов в полученной странице.
			* `content` - Массив публикаций (содержание страницы).
			Подробнее см. [Публикации#getPostById](#/Публикации/getPostById).
			* `sort` - Метаданные о сортировке.
				* `empty` - Пустой ли параметры сортировки.
				* `sorted` - Отсортирован ли элементы.
				* `unsorted` - Обратное к `sorted`.
			* `pageable` - Метаданные о странице.
				* `sort` - Эквивалентен `sort`.
				* `offset` - Смещение, необходимое для выборки определённого \
				подмножества элементов. Равен по формуле `pageSize * pageNumber`.
				* `pageNumber` - Эквивалентен `number`.
				* `pageSize` - Эквивалентен `size`.
				* `paged` - Разбита ли элементы (`content`) на страницы.
				* `unpaged` - Обратное к `paged`.
			* `first` - Является ли полученная страница первой.
			* `last` - Является ли полученная страница последней.
			* `empty` - Пустая ли полученная страница.
			""";
